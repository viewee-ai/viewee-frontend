...
test("Interview Session Performance Test", async ({ page }) => {
  ...
  await page.goto(
    `http://localhost:3000/dashboard/interview?title=${encodedTitle}`
  );
        ...
        fetch("/api/stt_token", {
          ...
          .then((token) => {
            ...
            socket.onopen = () => {
              ...
              const binaryData = Uint8Array.from(atob(audioBase64), (c) =>
                c.charCodeAt(0)
              );
          ...
          .catch((err) => {
            if (err instanceof Error) {
              ...
              reject("An unexpected error occurred."); // Fallback for unknown errors
            }
          });
      });
    },
    { audioBase64: audio.toString("base64"), sessionId }
  );
...
`;
  ...
  const feedback = await page.evaluate(
    async ({ transcript, code, sessionId }) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch feedback: ${response.status} - ${responseText}`
        );
      }
  ...
  const ttsAudioDuration = await page.evaluate(
    async ({ sessionId }) => {
